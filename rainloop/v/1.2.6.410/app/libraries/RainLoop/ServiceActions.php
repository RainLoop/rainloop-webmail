...
class ServiceActions
{
	...
	public function ServiceAjax()
	{
		...
		{
			$bLogout = ('Folders' === $sAction && $oException instanceof \RainLoop\Exceptions\ClientException);
			if ($bLogout)
			{
				$this->oActions->ClearAuthToken();
			}

			$aResponseItem = $this->oActions->ExceptionResponse(
				empty($sAction) ? 'Unknown' : $sAction, $oException);

			if ($bLogout && \is_array($aResponseItem))
			{
		...
		if (!empty($this->aPaths[4]))
		{
			...
			if (0 === \strlen($sResult))
			{
				try
				{
					...
					if ($bCustom && $oAccount)
					{
						$mData = $this->oActions->StorageProvider()->Get($oAccount, StorageType::USER, 'CustomThemeBackground', '');
	...
	public function ServiceExternalLogin()
	{
		...
		$this->oActions->Location(APP_SHORT_INDEX_FILE_NAME);
		return '';
	}
	...
	public function ServiceChange()
	{
		if ($this->Config()->Get('webmail', 'allow_additional_accounts', true))
		{
			...
			if ($oAccountToLogin)
			{
				$this->oActions->AuthProcess($oAccountToLogin, true);
			}
		}

...
class Actions
{
	...
	private function compileLogFileName()
	{
		...
		if (false !== \strpos($sFileName, '{labs:'))
		{
			$sFileName = \preg_replace_callback('/\{labs:rand:([1-9])\}/', function ($aMatch) {
				return \mt_rand(\pow(10, $aMatch[1] - 1), \pow(10, $aMatch[1]) - 1);
			}, $sFileName);
	...
	public function ContactsProvider()
	{
		...
		{
			$this->oContactsProvider = new \RainLoop\Providers\Contacts($this->fabrica('contacts'));
		}
		...
		if (!empty($sToken))
		{
			...
			if (!empty($aAccountHash[0]) && 'token' === $aAccountHash[0] && 8 === \count($aAccountHash))
			{
				$oAccount = $this->LoginProvider()->Provide(
					$aAccountHash[1], $aAccountHash[2], $aAccountHash[3], '1' === $aAccountHash[4], empty($aAccountHash[5]) ? '' : $aAccountHash[5]);
	...
	public function AppData($bAdmin, $sAuthAccountHash = '')
	{
		...
		$aResult = array(
			...
			'IndexFile' => \APP_SHORT_INDEX_FILE_NAME,
			'Plugins' => array()
		);
		...
		$aResult['ProjectHash'] = md5($aResult['AccountHash'].APP_VERSION);
		...
		$aResult['PluginsLink'] = APP_SHORT_INDEX_FILE_NAME.'?/Plugins/0/'.
			($bAdmin ? 'Admin' : 'User').'/'.$sStaticCache.'/';
		$aResult['NewThemeLink'] = APP_SHORT_INDEX_FILE_NAME.'?/Css/0/'.
			($bAdmin ? 'Admin' : 'User').'/-/'.($bAdmin ? 'Default' : $aResult['Theme']).'/-/'.$sStaticCache.'/';
		$aResult['LangLink'] = APP_SHORT_INDEX_FILE_NAME.'?/Lang/0/'.
	 ...
	 * @throws \RainLoop\Exceptions\ClientException
	 */
	public function LoginProcess($sEmail, $sLogin, $sPassword, $sSignMeToken = '', $bPasswordIsXOAuth2 = false)
	{
		...
		$oAccount = $this->LoginProvider()->Provide($sEmail, $sLogin, $sPassword, $bPasswordIsXOAuth2, $sSignMeToken);
		if (!($oAccount instanceof \RainLoop\Account))
		{
	...
	public function DoLogin()
	{
		...
		$oAccount = $this->LoginProcess($sEmail, $sLogin, $sPassword, $sSignMeToken, false);
	...
	public function DoAccountAdd()
	{
		...
		$oNewAccount = $this->LoginProcess($sEmail, $sLogin, $sPassword, false, false);
...
//		$oAccount = $this->getAccountFromToken();
		...
		return $this->DefaultResponse(__FUNCTION__, array(
			...
			'UserStatistic' => $bUserStatistic ? $this->KeenIO('Activity') : false
		));
	}
	...
	public function DoAdminDomainList()
	{
		...
		return $this->DefaultResponse(__FUNCTION__,
			$this->DomainProvider()->GetList($iOffset, 20, $sSearch));
	}
	...
	{
		return @file_exists(APP_INDEX_ROOT_PATH.APP_INDEX_FILE_NAME) && @is_writable(APP_INDEX_ROOT_PATH.APP_INDEX_FILE_NAME);
	}
...
//		));
	...
	public function DoSettingsUpdate()
	{
		...
		$this->setSettingsFromParams($oSettings, 'MPP', 'int', function ($iMpp) {
			return (int) (in_array($iMpp, array(10, 20, 30, 50, 100/*, 150, 200*/)) ? $iMpp : 20);
		});
					...
					if ($oRcpt && 0 < $oRcpt->Count())
					{
						try
						{
							...
							if ($oAccount->Domain()->OutAuth())
							{
								if ($oAccount->PasswordIsXOAuth2())
								{
									$oSmtpClient->LoginWithXOauth2($oAccount->Password());
								}
								else
								{
...
//		}
	...
	public function KeenIO($sName, $aData = array())
	{
		...
		if (APP_VERSION === APP_DEV_VERSION ||
			\in_array(APP_SITE, array('localhost', 'demo.rainloop.net', 'demo.rainloop.de')))
		{
			return true;
		}
		...
		try
		{
			...
			$this->Logger()->Write('KeenIO: addEvent = '.$sName);
			return true;
		}
		catch (\Exception $oException)
		{
			$this->Logger()->WriteException($oException, \MailSo\Log\Enumerations\Type::WARNING);
		}
	...
	public function ErrorTemplates($sTitle, $sDesc, $bShowBackLink = true)
	{
		return strtr(file_get_contents(APP_VERSION_ROOT_PATH.'app/templates/Error.html'), array(
			...
			'{{BackHref}}' => APP_SHORT_INDEX_FILE_NAME
		));
	}
...
//					'ReadingConfirmation' => $mResponse->ReadingConfirmation(),
				...
				$mResult['HasAttachments'] = $oAttachments && 0 < $oAttachments->Count();

...
a.allowCustomTheme=f.observable(!0);a.allowAdditionalAccounts=f.observable(!0);a.messagesPerPage=f.observable(n.Defaults.MessagesPerPage);a.mainMessagesPerPage=a.messagesPerPage;a.mainMessagesPerPage=f.computed({read:a.messagesPerPage,write:function(b){-1<d.inArray(d.pInt(b),n.Defaults.MessagesPerPageArray)?b!==a.messagesPerPage()&&a.messagesPerPage(b):a.messagesPerPage.valueHasMutated()}});a.facebookEnable=f.observable(!1);a.facebookAppID=f.observable("");a.facebookAppSecret=f.observable("");a.twitterEnable=
f.observable(!1);a.twitterConsumerKey=f.observable("");a.twitterConsumerSecret=f.observable("");a.googleEnable=f.observable(!1);a.googleClientID=f.observable("");a.googleClientSecret=f.observable("");a.dropboxEnable=f.observable(!1);a.dropboxApiKey=f.observable("");a.contactsIsSupported=f.observable(!1);a.contactsIsAllowed=f.observable(!1)};d.createMomentDate=function(a){return f.computed(function(){Ba();return this.moment().fromNow()},a)};d.createMomentShortDate=function(a){return f.computed(function(){var a=
"",a=ca(),c=this.moment(),l=this.momentDate();return a=4>=a.diff(c,"hours")?l:a.format("L")===c.format("L")?d.i18n("MESSAGE_LIST/TODAY_AT",{TIME:c.format("LT")}):a.clone().subtract("days",1).format("L")===c.format("L")?d.i18n("MESSAGE_LIST/YESTERDAY_IN",{TIME:c.format("LT")}):a.year()===c.year()?c.format("D MMM."):c.format("LL")},a)};d.isFolderExpanded=function(a){var b=e.local().get(g.ClientSideKeyName.ExpandedFolders);return i.isArray(b)&&-1!==i.indexOf(b,a)};d.setExpandedFolder=function(a,b){var c=
...
j;i++)if((r=a.Result["@Collection"][i])&&"Object/Message"===r["@Object"]){G=n[i];if(!G||!G.initByJson(r))G=m.newInstanceFromJson(r);G&&(f.hasNewMessageAndRemoveFromCache(G.folderFullNameRaw,G.uid)&&5>=s&&(s++,G.newForAnimation(!0)),G.deleted(!1),b?e.cache().initMessageFlagsFromCache(G):e.cache().storeMessageFlagsToCache(G),G.lastInCollapsedThread(h&&-1<d.inArray(d.pInt(G.uid),h)?!0:!1),o.push(G))}c.messageListCount(k);c.messageListSearch(d.isNormal(a.Result.Search)?a.Result.Search:"");c.messageListEndSearch(d.isNormal(a.Result.Search)?
a.Result.Search:"");c.messageListEndFolder(d.isNormal(a.Result.Folder)?a.Result.Folder:"");c.messageListPage(Math.ceil(q/c.messagesPerPage()+1));c.messageList(o);c.messageListIsNotCompleted(!1);(G=c.message())&&c.messageList.setSelectedByUid&&c.messageList.setSelectedByUid(G.generateUid());n.length<o.length&&(c.staticMessageList=o);f.clearNewMessageCache();p&&(b||u||e.data().useThreads())&&e.folderInformation(p.fullNameRaw,o)}else e.data().messageListCount(0),e.data().messageList([]),e.data().messageListError(d.getNotification(a&&
a.ErrorCode?a.ErrorCode:g.Notification.CantGetMessageList))};da.prototype.oRequests={};da.prototype.defaultResponse=function(a,b,c,f,j,t){var v=function(){g.StorageResultType.Success!==c&&Ua&&(c=g.StorageResultType.Unload);if(g.StorageResultType.Success===c&&f&&!f.Result){if(f&&-1<d.inArray(f.ErrorCode,[g.Notification.AuthError,g.Notification.AccessError,g.Notification.ConnectionError,g.Notification.DomainNotAllowed,g.Notification.AccountNotAllowed,g.Notification.MailServerError,g.Notification.UnknownNotification,
g.Notification.UnknownError])&&Ga++,f&&g.Notification.InvalidToken===f.ErrorCode&&Ha++,10<Ha&&e.loginAndLogoutReload(!0),f.Logout||7<Ga)h.__rlah_clear&&h.__rlah_clear(),e.loginAndLogoutReload(!0)}else g.StorageResultType.Success===c&&(f&&f.Result)&&(Ha=Ga=0);a&&(x.runHook("ajax-default-response",[b,g.StorageResultType.Success===c?f:null,c,j,t]),a(c,g.StorageResultType.Success===c?f:null,j,b,t))};switch(c){case "success":c=g.StorageResultType.Success;break;case "abort":c=g.StorageResultType.Abort;
break;default:c=g.StorageResultType.Error}g.StorageResultType.Error===c?i.delay(v,300):v()};da.prototype.ajaxRequest=function(a,b,c,f,g){var t=this,v=""===f,k=(new h.Date).getTime(),m=null,o="",b=b||{},c=d.isNormal(c)?c:2E4,f=d.isUnd(f)?"":d.pString(f),g=d.isArray(g)?g:[];(o=b.Action||"")&&0<g.length&&i.each(g,function(a){t.oRequests[a]&&(t.oRequests[a].__aborted=!0,t.oRequests[a].abort&&t.oRequests[a].abort(),t.oRequests[a]=null)});v&&(b.XToken=e.settingsGet("Token"));m=j.ajax({type:v?"POST":"GET",
url:e.link().ajax(f),async:!0,dataType:"json",data:v?b:{},headers:{},timeout:c,global:!0});m.always(function(c,e){var f=!1;c&&c.Time&&(f=d.pInt(c.Time)>(new h.Date).getTime()-k);o&&t.oRequests[o]&&(t.oRequests[o].__aborted&&(e="abort"),t.oRequests[o]=null);t.defaultResponse(a,o,e,c,f,b)});o&&(0<g.length&&-1<d.inArray(o,g))&&(this.oRequests[o]&&(this.oRequests[o].__aborted=!0,this.oRequests[o].abort&&this.oRequests[o].abort(),this.oRequests[o]=null),this.oRequests[o]=m);return m};da.prototype.defaultRequest=
...
"")+this.settingsGet("Title")||"";a!==h.document.title&&(h.document.title=a)};J.prototype.loginAndLogoutReload=function(a,b){var c=d.pString(this.settingsGet("CustomLogoutLink")),f=!!this.settingsGet("InIframe"),a=d.isUnd(a)?!1:!!a,b=d.isUnd(b)?!1:!!b;a&&(b&&h.close)&&h.close();a&&""!==c&&h.location.href!==c?i.defer(function(){f&&h.parent?h.parent.location.href=c:h.location.href=c}):(q.routeOff(),q.setHash(e.link().root(),!0),q.routeOff(),i.defer(function(){f&&h.parent?h.parent.location.reload():
